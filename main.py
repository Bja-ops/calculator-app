...
def mult_nums():
    try:
        ...
        b = float(entry6.get())
        wynik = a * b
        mult_label.config(text=f"Wynik: {wynik}")
    except ValueError:
        messagebox.showerror("Error", "Give right data!")
...
tk.Label(root, text="Substraction").grid(row=5, column=0, columnspan=2, pady=10)
...
tk.Button(root, text="Sub", command=sub_nums).grid(row=8, column=0, columnspan=2, pady=5)
sub_label = tk.Label(root, text="Result: ")
sub_label.grid(row=9, column=0, columnspan=2, pady=5)

tk.Label(root, text="Multiplying").grid(row=0, column=3, columnspan=2, pady=5)
